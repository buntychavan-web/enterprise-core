...
export const Route = createFileRoute("/_app/manager-review/$id")({
  head: ({ params }) => ({
    meta: [{ title: `Manager Review ${params.id} — EWOS` }, { name: "robots", content: "noindex" }],
  }),
...
function ManagerReviewPage() {
  ...
  const submit = async () => {
    ...
    setSubmitting(true);
    try {
      const updated = await performanceSelfServiceApi.submitManager(appraisal.id, {
        rating: value,
      ...
      toast.error(err instanceof ApiError ? err.message : "Failed to submit manager review.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <PageHeader eyebrow="Team Performance" title="Manager Review" />

      {notFound ? (
        <EmptyState
          title="Appraisal not found"
          description="This appraisal doesn't exist or you're not the manager on it."
        />
      ) : error ? (
        ...
        <>
          <Card>
            ...
            <CardContent className="space-y-2">
              ...
              {!appraisal.selfComments && !appraisal.selfRating && (
                <p className="text-sm text-muted-foreground">No self review recorded yet.</p>
              )}
            </CardContent>
          </Card>
          ...
          {appraisal.status !== "PENDING_MANAGER" ? (
            <EmptyState
              title="Not awaiting manager review"
      ...
      <AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Once submitted, this appraisal moves to the reviewer stage and your rating can no
              longer be edited.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
